...
class NewsFilter:
    ...
    def __init__(self):
        ...
        if os.path.exists(cache_file):
            ...
            if file_age < 21600: # 6 hours
                self.last_successful_fetch = time.time() # Pretend we just fetched it to delay next fetch
                ...
                try:
                    ...
                    if isinstance(data, list):
                        for event in data:
                            if event.get('country') == target_currency and event.get('impact') == impact_level:
    ...
    def _fetch_data(self):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as response: # Added timeout to prevent hanging
            ...
            print(f"[WARNING] [NEWS FILTER] API Fetch failed ({e}). Attempting to load from local cache...")
            if os.path.exists(cache_file):
                try:
                    with open(cache_file, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                ...
                print("[ERROR] [NEWS FILTER] No local cache available. Filter temporarily disabled.")
        ...
        try:
            ...
            if parsed_events:
                print(f"[INFO] [NEWS FILTER] Successfully loaded {len(parsed_events)} high-impact {target_currency} events for the week.")
        except Exception as e:
