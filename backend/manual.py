...
def main():
    ...
    if symbol_info:
        ...
        if filling_mode & 1:
            ...
            type_filling = 2
        tick_size = (
            symbol_info.trade_tick_size if symbol_info.trade_tick_size > 0 else 0.01
        )
    ...
    if getattr(config, "ENABLE_DYNAMIC_SL_TP", False):
        analysis = TechnicalIndicators.analyze_basic_timeframe(
            symbol, mt5.TIMEFRAME_M5, bars=100
        )
        atr_50 = analysis.get("atr_50", 2.50)
        if atr_50 > 0:
            dynamic_tp = max(
                dynamic_tp, atr_50 * getattr(config, "DYNAMIC_TP_BASE_MULTIPLIER", 2.0)
            )
    ...
    risk_pts = min(
        risk_pts, round(dynamic_tp * getattr(config, "MAX_RISK_TO_TP_RATIO", 1.0), 2)
    )

    # Default manual stop loss is 10.00 points
    risk_pts = 10.00
    ...
    hard_sl = round(
        round(
            (price - risk_pts if direction == "BUY" else price + risk_pts) / tick_size
        )
        * tick_size,
        digits,
    )
    ...
    broker_tp = round(
        round(
            (price + dynamic_tp if direction == "BUY" else price - dynamic_tp)
            / tick_size
        )
        * tick_size,
        digits,
    ...
    if symbol_info:
        ...
        if direction == "BUY":
            ...
            if hard_sl != 0.0 and hard_sl > sl_limit:
                hard_sl = sl_limit
            if broker_tp != 0.0 and broker_tp < tp_limit:
                broker_tp = tp_limit
        else:
            sl_limit = round(price + min_dist, digits)
            tp_limit = round(price - min_dist, digits)
    ...
    print(
        f"⏳ Sending {num_trades} MANUAL {direction}(s) for {volume} {symbol} @ {price:.2f}..."
    )
    ...
    for i in range(num_trades):
        ...
        if result is None:
            ...
            print(
                f"✅ MANUAL {direction} {i+1}/{num_trades} PLACED! (Ticket: {result.order})"
            )

    print(
        ">> Look at your main bot terminal — it should instantly detect this and apply the Trailing SL!"
    )
    mt5.shutdown()
