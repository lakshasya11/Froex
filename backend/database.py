...
class TradeDatabase:
    ...
    def __init__(self, db_name="trade_journal.db"):
        ...
        self.worker_thread = threading.Thread(target=self._bg_writer_worker, daemon=True)
    ...
    def init_db(self):
        ...
        try:
            ...
            # Migration: Add ticket column dynamically if database already exists but has no ticket column
            try:
                ...
                cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_trades_ticket ON trades(ticket) WHERE ticket IS NOT NULL")
                conn.commit()
                print("[INFO] [DATABASE] Added ticket column to existing trades table.")
            except sqlite3.OperationalError:
                pass

            # Migration: Add volume column
            try:
    ...
    def _bg_writer_worker(self):
        ...
        conn = self._get_connection()
        print("[INFO] [DB WORKER] Database background writer thread started successfully.")
                    ...
                    try:
                        with conn:
                            ...
                            if ticket_val is not None:
                                try: ticket_val = int(ticket_val)
                                except (ValueError, TypeError): ticket_val = None
                        ...
                        if "locked" in str(e).lower():
                            print(f"[WARNING] [DB WORKER] Database locked. Retrying attempt {attempt+1}/3...")
                            time.sleep(0.2)
                        else:
                            raise e
                
                if not success:
                    print(f"[CRITICAL] [DB WORKER] Failed to save trade data for ticket {data.get('ticket')} due to persistent locking.")
                
                self.write_queue.task_done()
                
            except Exception as e:
                print(f"[ERROR] [DB WORKER] Exception in background database thread: {e}")
        ...
        try:
            ...
            if date_filter is not None:
                if date_filter == "today":
                    ...
                    cursor.execute("SELECT COUNT(*) FROM trades WHERE entry_time LIKE ?", (f"{today_local}%",))
            ...
            cursor.execute(f"{cte} SELECT COUNT(*) FROM {table_name}{query_where}", params)
            ...
            cursor.execute(f"{cte} SELECT COUNT(*) FROM {table_name}{query_where}" + (" AND " if query_where else " WHERE ") + "profit_dollars > 0", params)
            ...
            cursor.execute(f"{cte} SELECT SUM(profit_dollars) FROM {table_name}{query_where}", params)
            total_profit = cursor.fetchone()[0] or 0.0
            
            # Gross profit (Sum of all positive winning trades)
            cursor.execute(f"{cte} SELECT SUM(profit_dollars) FROM {table_name}{query_where}" + (" AND " if query_where else " WHERE ") + "profit_dollars > 0", params)
            ...
            # Profit Rate (replaces traditional Win Rate)
            abs_losses = abs(total_losses)
            win_rate = (gross_profit / (gross_profit + abs_losses)) * 100 if (gross_profit + abs_losses) > 0 else 0.0
    ...
    def auto_import_from_csv(self, csv_path):
        ...
        resolved_csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), csv_path)
        ...
        try:
            ...
            with open(resolved_csv_path, mode='r', encoding='utf-8') as f:
                ...
                for row in csv.DictReader(f):
                    try: ticket_val = int(row.get('ticket')) if row.get('ticket') else None
                    except (ValueError, TypeError): ticket_val = None
                ...
                if trades_to_import:
                    ...
                    """, trades_to_import)
                    conn.commit()
                    print(f"[SUCCESS] [DATABASE] Successfully imported {len(trades_to_import)} historical trades into SQLite!\n")
