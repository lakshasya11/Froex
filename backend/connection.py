...
class MT5Connection:
    ...
    def initialize_mt5():
        ...
        if not mt5.initialize(
            path=mt5_path, login=mt5_login, password=mt5_pass, server=mt5_server
        ):
    ...
    def ensure_connection():
        ...
        print(
            "⚠️ MT5 connection lost. Initiating reconnect sequence with exponential backoff..."
        )
        ...
        while True:
            ...
            print(
                f"Reconnect failed. Retrying in {total_sleep:.2f}s (Attempt {attempt + 1})..."
            )
