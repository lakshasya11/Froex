...
import time
import config
from config import SPREAD_ALLOWANCE, TRAIL_TRIGGER_PTS, TRAIL_GAP_PTS, TRAIL_MODIFY_MIN_INTERVAL
...
class ExitMixin:
    ...
    def check_exit_conditions(self, tick, analysis, positions):
        ...
        for pos in positions:
            ...
            current_profit = (tick.bid - entry_price) if direction == "BUY" else (entry_price - tick.ask)
            ...
            for trigger_pts, lock_pts in getattr(config, 'PROFIT_LOCK_STEPS', []):
            ...
            if locked_sl_price is not None:
                lock_price_rounded = round(round(locked_sl_price / tick_size) * tick_size, digits)
                prev_lock = pos_data.get('price_lock_sl_price')
                is_valid_ratchet = (
                    prev_lock is None or
                    (direction == "BUY"  and lock_price_rounded > prev_lock) or
                    (direction == "SELL" and lock_price_rounded < prev_lock)
                )
                if is_valid_ratchet:
                    pos_data['price_lock_sl_price'] = lock_price_rounded
                    if prev_lock != lock_price_rounded:
                        self.log(f"🔒 PROFIT LOCK ENGAGED #{ticket} | Lock SL: {lock_price_rounded:.2f}", self.Colors.CYAN)
            ...
            if price_lock_sl is not None:
                lock_hit = (
                    (direction == "BUY"  and tick.bid <= price_lock_sl) or
                    (direction == "SELL" and tick.ask >= price_lock_sl)
                )
                if lock_hit:
                    self.log(f"🔒 PRICE LOCK BREACHED #{ticket} | Exiting at {price_lock_sl:.2f}", self.Colors.ORANGE)
            ...
            if hard_sl_sw is not None and hard_sl_sw != 0.0:
                if (direction == "BUY" and tick.bid <= hard_sl_sw) or \
                   (direction == "SELL" and tick.ask >= hard_sl_sw):
                    self.log(f"⚠️ HARD SL HIT #{ticket} (SW) | SL={hard_sl_sw:.2f}", self.Colors.ORANGE)
                    pos_data['_closing'] = True
            ...
            _tp_target     = pos_data.get('initial_tp_pts', TRAIL_TRIGGER_PTS)
            _trail_trigger = TRAIL_TRIGGER_PTS
            _trail_gap     = TRAIL_GAP_PTS
            ...
            if pos_data.get('live_trail_active', False):
                ...
                target_price = (entry_price + calculated_trail_pts) if direction == "BUY" else (entry_price - calculated_trail_pts)
                intended = round(round(target_price / tick_size) * tick_size, digits)

            if intended is not None:
                software_cur = pos_data.get('trail_sl_price')
                is_better = (
                    (direction == "BUY"  and (software_cur is None or intended > software_cur)) or
                    (direction == "SELL" and (software_cur is None or intended < software_cur))
                )
                if is_better:
            ...
            if trail_sl is not None:
                ...
                if (now_t - last_modify) >= TRAIL_MODIFY_MIN_INTERVAL:
                    trail_sl_rounded = round(round(trail_sl / tick_size) * tick_size, digits)

                    # Fetch LIVE position SL (not stale snapshot) for accurate comparison
                    _live_pos_list = mt5.positions_get(ticket=pos.ticket)
                    _live_sl = float(_live_pos_list[0].sl) if _live_pos_list else float(pos.sl or 0)
                    ...
                    if needs_update:
                        ...
                        if actual_sl:
                            ...
                            if abs(actual_sl - last_sent) >= (tick_size * 0.5):
                                self.log(f"🔄 TRAIL SL MT5 Sync → {actual_sl:.2f}", self.Colors.CYAN if direction == "BUY" else self.Colors.MAGENTA)
                ...
                if (direction == "BUY"  and tick.bid <= trail_sl) or \
                   (direction == "SELL" and tick.ask >= trail_sl):
                    self.log(f"⚠️ TRAIL SL HIT #{ticket} | SL={trail_sl:.2f}", self.Colors.ORANGE)
                    pos_data['_closing'] = True
    ...
    def close_position(self, pos, reason):
        ...
        self.log(f"📤 CLOSE REQUEST | Ticket:{pos.ticket} | Reason:{reason}", self.Colors.CYAN)
        ...
        request = {
            ...
            "magic": 123456, "comment": exit_comment,
            "type_time": mt5.ORDER_TIME_GTC, "type_filling": type_filling,
        }
        result = mt5.order_send(request)

        if result is None:
            self.log("⚠️ Close attempt failed. Retrying alternate filling mode...", self.Colors.YELLOW)
            request["type_filling"] = 1 if type_filling != 1 else 0
            result = mt5.order_send(request)

        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            self.log(f"✅ CLOSE SUCCESS | Ticket:{pos.ticket} | Reason:{reason}", self.Colors.GREEN)
        ...
        if retcode in [10036, 10013, 10018]:
            self.log(f"ℹ️ Ticket {pos.ticket} already handled by server.", self.Colors.YELLOW)
            if pos.ticket in self.position_data:
                self.position_data[pos.ticket]['exit_reason'] = reason
            return True

        self.log(f"❌ CLOSE CRITICAL FAILURE | Ticket:{pos.ticket} | Code:{retcode}", self.Colors.RED)
        return False
